...
class CustomIntentHandler extends IntentHandler {
  ...
  getSites(target, config) {
    const sites = [];
    sites.push(config['iplink:iplink']['endpoint-a']['ne-id']);
    sites.push(config['iplink:iplink']['endpoint-b']['ne-id']);
    return sites;
  }
  
  getSiteParameters(intentType, intentTypeVersion, target, config, siteNames) {
    const sites = [];
    sites.push(config['iplink:iplink']['endpoint-a']);
    sites.push(config['iplink:iplink']['endpoint-b']);
  ...
  getGlobalParameters(intentType, intentTypeVersion, target, config) {  
    ...
    const global = config['iplink:iplink'];

    global.cid = target;
    global.testId = parseInt(global.cid.match(/\d+/));

    if (global.description)
      global.description = `iplink: ${global.description}`;
  ...
  validateHook(intentType, intentTypeVersion, target, config, contextualErrorJsonObj) {
    const siteA = config['iplink:iplink']['endpoint-a']['ne-id'];
    const siteB = config['iplink:iplink']['endpoint-b']['ne-id'];
