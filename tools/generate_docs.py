...
def load_galaxy_config() -> Dict:
    """Load namespace and name from galaxy.yml."""
...
def generate_doc_md(item_file: Path, item_type: str) -> str:
    ...
    for opt_name, opt_data in options.items():
        desc_lines = opt_data.get('description', [])
        description_text = ' '.join(desc_lines)[:80]
...
def generate_all_docs():
    ...
    if MODULE_DIR.exists():
        ...
        modules = [m for m in modules if not m.name.startswith("_")]
        
        for module_file in modules:
            md_content = generate_doc_md(module_file, 'module')
            output_file = DOCS_REFERENCE_DIR / f"module-{module_file.stem}.md"
    ...
    for plugin_type in PLUGIN_TYPES:
        ...
        for plugin_file in plugins:
            ...
            output_file = DOCS_REFERENCE_DIR / f"plugin-{plugin_file.stem}.md"
...
def generate_combined_index(modules: List[Path], plugins: List[Path]):
...
def update_mkdocs_nav(modules: List[Path], plugins: List[Path]):
    ...
    # Add Modules section if modules exist
    if modules:
        modules_nav = []
        for module_file in sorted(modules, key=lambda m: m.stem):
            module_name = module_file.stem
            display_name = format_display_name(module_name)
            modules_nav.append({display_name: f'reference/module-{module_name}.md'})
        
        reference_nav['Reference'].append({'Modules': modules_nav})
    
    # Add Plugins section if plugins exist
    if plugins:
        plugins_nav = []
        for plugin_file in sorted(plugins, key=lambda p: p.stem):
            plugin_name = plugin_file.stem
            display_name = format_display_name(plugin_name)
            plugins_nav.append({display_name: f'reference/plugin-{plugin_name}.md'})
